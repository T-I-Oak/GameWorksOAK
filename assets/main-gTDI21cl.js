(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))n(e);new MutationObserver(e=>{for(const s of e)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function t(e){const s={};return e.integrity&&(s.integrity=e.integrity),e.referrerPolicy&&(s.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?s.credentials="include":e.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(e){if(e.ep)return;e.ep=!0;const s=t(e);fetch(e.href,s)}})();const u=async(o,i={})=>{const t=await fetch(o,i);if(!t.ok)throw new Error(`CommonFetch: Failed to fetch "${o}". Status: ${t.status} ${t.statusText}`);try{return await t.json()}catch{throw new Error(`CommonFetch: Failed to parse JSON from "${o}".`)}};async function p(){const o=document.getElementById("portal-version");o&&(o.textContent="v0.9.0"),await y(),L()}async function y(){const o=document.getElementById("gamesGrid");o.innerHTML='<div style="grid-column: 1/-1; text-align: center; padding: 4rem; color: var(--text-muted);">Loading projects...</div>';const i=await u("data/project_list.json"),t=await Promise.all(i.map(async n=>{const e=n.id,s=n.title,r=`https://t-i-oak.github.io/${e}/`;let a=null;const l=m("data/project_info.json",r);try{const c=await fetch(l);c.ok&&(a=await c.json(),a.isMaintenance=!1)}catch{}if(!a)a={id:e,title:s,isMaintenance:!0,image:"",badge:{content:"",type:"none"},tags:["データ取得不可"],description:"プロジェクト情報の取得に失敗しました。一時的なメンテナンス中か、ネットワーク環境に問題がある可能性があります。",button:{content:"UNAVAILABLE",url:"javascript:void(0)",type:"pending"}};else if(a.id=e,a.button?a.button.url||(a.button.url=r):a.button={url:r,content:"PLAY NOW",type:"published"},a.logo&&a.logo.path){const c=m(a.logo.path,r),d=await fetch(c);d.ok&&(a.logo.content=await d.text())}return a}));v(t.filter(n=>n!==null))}function v(o){const i=document.getElementById("gamesGrid");i.innerHTML=o.map((t,n)=>{const e=t.logo,s=t.badge,r=t.button,a=t.isMaintenance;let l="";e&&e.content?l=`<div class="game-logo-wrapper Logo${e.type?e.type.charAt(0).toUpperCase()+e.type.slice(1):"Standard"}">${e.content}</div>`:l=`<div class="game-logo-wrapper LogoText"><h3>${t.title}</h3></div>`;const c=r.type==="pending"||a,d=c?"javascript:void(0)":r.url,f=c?'onclick="return false;"':"";return`
